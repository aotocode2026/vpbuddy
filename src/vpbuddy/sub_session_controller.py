    ...
    if sid not in _AGENT_CACHE:
        if _AGENT_AVAILABLE and _AIAgent is not None:
            ...
            try:
                ...
                "",
                "【铁律 — 数据隔离】",
                "- ⚠️ 你完全不知道本系统(VPBuddy/hermes/funasr/sqlite-vec)的存在。你只是一个文档生成助手。",
                "- ⚠️ 不准在输出中提及任何系统内部名词",
                "- ⚠️ 如果会议内容不足（转写文本 < 100 字且无明确需求），**不生成任何内容**，输出「内容不足,等待更多发言」",
                "- ⚠️ v0.22.8: 严禁读取/提及服务器文件系统路径、主机用户名(/home/xxx)、环境变量。你是独立助手,不应感知宿主机信息。",
                "- ⚠️ v0.22.8: 禁止用终端工具探索系统信息(whoami/uname/hostname/df/ps/id/env)。这些与会议文档生成无关。",
                "- ⚠️ v0.22.8: 种子/示例数据严禁包含可能与开发者/用户身份有关的姓名、邮箱、路径。只用中性占位符。",
                "",
                "工作流:",
        ...
        if doc_kind == "demo":
            try:
                ...
                if v_result.get("ok"):
                    ...
                    result["demo_placeholder_skipped"] = True
                    # 推 doc-update 用 status=placeholder 告知前端可以忽略
                    try:
                        from .realtime_server import push_event
                        push_event(meeting_id, "doc-update", {
                            "kind": doc_kind,
                        ...
                        logger.warning(f"[{meeting_id}/demo] push SSE placeholder failed: {e}")
                    return result
                else:
